...
<html>
	...
	<body>
		...
		<div id="list-item" class="content scaffold-list" role="main">
            <h1><g:message code="default.show.label" args="[entityName]" />   <g:helpBalloon code="item.label" mostra="true"/></h1>
			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="tipo" title="${message(code: 'item.tipo.label', default: 'Tipo')}" />
				...
				<g:each in="${itemInstanceList}" status="i" var="itemInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						<g:if test="${session.estab.documento.equals('101010')}">
						<td><g:link action="show" id="${itemInstance.id}">${fieldValue(bean: itemInstance, field: "descricao")}</g:link></td>
						</g:if>
						<g:else>
						<td>${fieldValue(bean: itemInstance, field: "descricao")}</td>
						</g:else>
						<td>${fieldValue(bean: itemInstance, field: "tipo")}</td>
					</tr>
				</g:each>
				</tbody>
			</table>
